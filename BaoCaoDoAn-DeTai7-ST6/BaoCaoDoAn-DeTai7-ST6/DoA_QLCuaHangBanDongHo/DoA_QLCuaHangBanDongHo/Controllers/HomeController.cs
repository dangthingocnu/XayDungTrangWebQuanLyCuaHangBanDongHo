...
namespace DoA_QLCuaHangBanDongHo.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult TimKiem(FormCollection c)
        {
            ...
            ViewBag.ThongBao = "Đã tìm thấy " + lst2.Count.ToString() + " quyển Sách :";
            return View(lst2);
        }
        public ActionResult HienThiLoai(int maloai, int? page)
        {
            int pageNumber = (page ?? 1);
            int pageSize = 9;
            List<SANPHAM> lst = dh.SANPHAMs.ToList();
            List<SANPHAM> lst2 = lst.Where(t => t.MaLoai==maloai).ToList();
            return View(lst2.OrderBy(n => n.MaSP).ToPagedList(pageNumber, pageSize));
        }
